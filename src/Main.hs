...
cube :: [LineSeg]
cube = (zipWith color [(cubeVerts !! a, cubeVerts !! b) | b <- [0..7], a <- [0..b], oneCompDiff (cubeVerts !! a) (cubeVerts !! b)] [0..]) ++ [LineSeg v (v &+ (Vec3 epsilon 0 0)) (Vec3 1000 1000 1000) | v <- cubeVerts]
    where
        epsilon = 1e-6
        comp n i = n `mod` (i * 2) >= i
        vcomp n i = if comp n i then 0.5 else -0.5
        cubeVerts = [Vec3 (vcomp n 1) (vcomp n 2) (vcomp n 4) | n <- [0..7]]
        ccomp n i = if comp n i then 255 else 127
        color (a, b) n = let n' = n `mod` 6 + 1 in LineSeg a b (Vec3 0 (ccomp n' 1) (ccomp n' 2))
    ...
    where
        pixel x y = PixelRGB8 (floor r) (floor g) (floor b)
            where
                ...
                Vec3 r g b = onComp min (Vec3 255 255 255) $ foldr1 (&+) [c &* (1 / ((lineRayDistance t l + 1) ^ 50)) | l@(LineSeg _ _ c) <- lineSegs]
